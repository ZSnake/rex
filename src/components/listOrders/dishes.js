...
const deleteTheDish = (dish, {deleteDishSuccess, deleteDishFailure,
  deleteDishRequest, user, history}) => {
    deleteDishRequest();
    return dishesService.deleteDish(user.token, dish)
    .then(response => response.json())
    .then(result => {
      deleteDishSuccess(result);
      history.push('/dishes');
    })
              ...
              {this.props.dishes.map(dish => (
                <tr key={dish.id}>
                  ...
                  <td className="center aligned">
                    ...
                    <button className="actionButton" onClick={() => deleteTheDish({
                      id: dish.id
                    })}>
